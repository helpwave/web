...
const TeamMember = ({
  ...
  return (
    <div className={clsx('flex flex-col h-full mb-8 w-1/2 text-center p-8 min-w-[300px] items-center')}>
      ...
      <h4 className={clsx('font-space text-gray-600 text-sm h-[32px]')}>{suffix}</h4>
      <h3 className={clsx('mt-4 text-2xl font-space')}>{role} <span className={clsx('text-md text-sm font-sans')}>{shorthand}</span></h3>
      <h4 className={clsx('mt-2 text-lg font-space leading-none justify-center flex flex-wrap gap-2 text-gray-600 max-w-[300px]')}> {generatedSocials} {hashTags}</h4>
    </div>
  )
}
