...
const defaultContactSectionTranslations: Record<Languages, ContactSectionLanguage> = {
  ...
  de: {
    ...
    MoreTitle: 'U want more?',
    MoreText:
      'Du kannst schon heute Teil der helpwave-Familie werden und uns auf unserem Weg zu einem innovativen Gesundheitswesen begleiten.',
    MoreTextForCTA: 'Melde dich dafür einfach direkt hier an!'
  }
}

const ContactSection = forwardRef<HTMLDivElement, PropsWithLanguage<ContactSectionLanguage, Record<string, unknown>>>(
    ...
    const handleSubmit = (e: FormEvent) => {
      ...
      if (email !== '' && firstName !== '' && lastName !== '' && message !== '') {
        hubspotSubmitForm({
          ...
          .then((json) => {
            // TODO: this may still be an errornous response, check the actual response for errors
            setEmail('')
            setFirstName('')
            setLastName('')
            setMessage('')
            ...
            toast.custom(
              <Toast message={language.successMessage} theme="dark" variant="primary" icon={<Send width={20} height={24} />} />,
              { position: 'bottom-left', duration: 1000 }
            )
          })
          .catch((error) => {
            console.error(error)
            toast.custom(<Toast message={language.failureMessage} theme="dark" variant="negative" icon={<AlertCircle />} />, {
          ...
          <div>
            <form onSubmit={handleSubmit}>
              ...
              <div className={tw('flex w-96')}>
                <textarea
                  className={tw(
                    'mt-1 block w-full h-48 bg-hw-dark-gray-800 placeholder:text-[#8E8E93] border-2 border-hw-primary-700 rounded-md shadow-sm focus:outline-none focus:border-indigo-500 focus:ring-indigo-500'
                  )}
                  value={message}
