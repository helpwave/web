      ...
      <Section ref={ref} id="features">
        ...
        <div className={tw("w-5/12 flex flex-col my-24")}>
          {features.map((value, index) =>
            <div className={tw(`${index % 2 == 0 ? "self-end mr-8" : "self-start ml-8"} w-9/12 flex flex-col my-8`)}>
              {Feature(value)}
            </div>)}
        </div>
      </Section>
      <div className={tw("w-6/12 h-full absolute right-0 top-0 rounded-3xl pb-16 pt-32 flex flex-col")}
           style={{background: "radial-gradient(#6F387999 15%, #281c2000 60%)"}}>
        <div className={tw("w-full h-3/6 mb-8 flex flex-row items-end justify-end")}>
          <div className={tw("bg-gray-200 rounded-3xl w-3/5 h-4/5 relative")}/>
          <div className={tw("bg-gray-200 w-3/12 h-full ml-8 mt-4 rounded-l-3xl")}/>
        </div>
        <div className={tw("w-full h-3/6 flex justify-end")}>
          <div className={tw("bg-gray-200 w-9/12 h-5/6 rounded-l-3xl")}/>
        </div>
      </div>
    </div>
  )
