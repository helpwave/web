...
const defaultStepsToDigitalizationSectionTranslation: Record<
  Languages,
  StepsToDigitalizationSectionTranslation
> = {
  en: {
    title: "\\secondary{Digital excellence} in\\newline 3 simple steps",
    description:
      "Our approach is to implement more efficient and fun\\newline processes in a simple yet powerful way.",
    step: "Step",
    step1Title: "Define the mission",
    step1Description:
      "At helpwave we are committed to driving unprecedented innovation. We believe in improving patient care by developing cutting-edge software in all areas of patient care.",
    step2Title: "Apply Open Innovation",
    step2Description:
      "We believe in the open source approach to ensure the highest quality work product. This unique approach allows our dedicated development team to focus on translating your requirements into innovative software features.",
    step3Title: "Aim for disruption",
    step3Description:
      "Regulatory burdens and high entry barriers make it difficult for small companies to enter the market, leading to a lack of competition. helpwave is here to change that. We provide a platform that invites everyone to the table, not just only the big players.",
  },
  de: {
    title: "\\secondary{Digitale Exelenz} in\\newline 3 Schritten",
    description:
      "Unser Ansatz ist es, effizientere Prozesse zu implementieren, die Spaß machen\\newline - und das auf einfache, aber wirkungsvolle Weise.",
    step: "Step",
    step1Title: "Definieren Sie die Mission", // TODO update
    step1Description:
      "Bei helpwave haben wir uns verpflichtet, beispiellose Innovationen voranzutreiben. Wir glauben an die Verbesserung der Patientenversorgung durch die Entwicklung innovativer Software in allen Bereichen der Patientenversorgung.",
    step2Title: "Offene Innovation anwenden",
    step2Description:
      "Wir glauben an den Open-Source-Ansatz, um die höchste Qualität des Arbeitsprodukts zu gewährleisten. Dieser einzigartige Ansatz ermöglicht es unserem engagierten Entwicklungsteam, sich auf die Umsetzung der Nutzer-Anforderungen in innovative Softwarefunktionen zu konzentrieren.",
    step3Title: "Disruption anstreben",
    step3Description:
      "Regulatorische Belastungen und hohe Eintrittsbarrieren erschweren den Markteintritt für kleine Unternehmen und führen zu mangelndem Wettbewerb. helpwave wird das ändern. Wir bieten eine Plattform, die jeden an den Tisch holt, nicht nur die großen Player.",
  },
};
...
export const StepsToDigitalizationSection = () => {
  const translation = useTranslation(
    defaultStepsToDigitalizationSectionTranslation
  );
  ...
  return (
    <SectionBase
      className={tw("flex flex-col gap-y-8 w-full !max-w-[1600px]")}
      outerClassName={tw("!px-0")}
    >
      <div className={tw("flex flex-col items-center text-center gap-y-2")}>
        <h2>
          <Span type="title" className={tw("!text-3xl")}>
            <MarkdownInterpreter text={translation.title} />
          </Span>
        </h2>
        <Span className={tw("font-space font-semibold")}>
          <MarkdownInterpreter text={translation.description} />
        </Span>
      </div>
      <Carousel
        hintNext={true}
