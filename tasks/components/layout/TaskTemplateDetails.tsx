...
} from "@helpwave/common/hooks/useTranslation";
...
const defaultTaskTemplateDetailsTranslations: Record<
  Languages,
  TaskTemplateDetailsTranslation
> = {
  en: {
    updateTaskTemplate: "Update Task Template",
    updateTaskTemplateDescription:
      "Here you can update details about the Task Template",
    ...
    tooShort: (minCharacters) =>
      `Too short, at least ${minCharacters} characters`,
...
export const TaskTemplateDetails = ({
  ...
  TaskTemplateDetailsTranslation,
  TaskTemplateDetailsProps
>) => {
  const context = useContext(TaskTemplateContext);

  const translation = useTranslation(
    defaultTaskTemplateDetailsTranslations,
    overwriteTranslation
  );
  ...
  const inputErrorClasses = tw(
    "border-hw-negative-500 focus:border-hw-negative-500 focus:ring-hw-negative-500 border-2"
  );
  const inputClasses = tw(
    "mt-1 block rounded-md w-full border-gray-300 shadow-sm focus:outline-none focus:border-hw-primary-500 focus:ring-hw-primary-500"
  );
  ...
  const nameErrorMessage: string | undefined = validateName(
    context.state.template.name
  );
  const isDisplayingNameError: boolean =
    touched.name && nameErrorMessage !== undefined;
  ...
  return (
    <div className={tw("flex flex-col py-4 px-6")}>
      ...
      <ColumnTitle
        title={
          isCreatingNewTemplate
            ? translation.createTaskTemplate
            : translation.updateTaskTemplate
        }
        subtitle={
          !isCreatingNewTemplate
            ? translation.updateTaskTemplateDescription
            : undefined
        }
      />
      <div className={tw(" flex flex-col gap-y-4 max-w-[400px] mb-4")}>
        <div>
          <Input
            ...
            className={tx(inputClasses, {
              [inputErrorClasses]: isDisplayingNameError,
            })}
          />
          {isDisplayingNameError && (
            <Span type="formError">{nameErrorMessage}</Span>
          )}
        </div>
        <Textarea
      ...
      />
      <div
        className={tx("flex flex-row mt-12", {
      ...
      >
        <Button
          ...
          onClick={() =>
            isCreatingNewTemplate
              ? onCreate(context.state.template)
