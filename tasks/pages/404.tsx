...
const NotFound: NextPage = ({ overwriteTranslation }: PropsForTranslation<NotFoundTranslation>) => {
  ...
  return (
    <PageWithHeader>
      ...
      <div className={clsx('desktop:w-5/12 h-full desktop:mx-auto tablet:mx-16 mobile:mx-8 relative z-[1]')}>
        <div className={clsx('absolute w-full top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center')}>
          ...
          <p className={clsx('text-3xl font-inter')}>{translation.notFoundDescription2} <Link className={clsx('underline text-primary hover:brightness-90')} href="/">{translation.homePage}</Link>.</p>
        </div>
      </div>
    </PageWithHeader>
  )
