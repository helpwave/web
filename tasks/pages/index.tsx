...
const Home: NextPage = () => {
  ...
  const { user, logout, accessToken } = useAuth(() =>
    router.push({ pathname: '/login', query: { back: true } })
  )

  const getWard = async (id: string) => {
    const wardService = new WardServiceClient(
      'https://staging-api.helpwave.de/task-svc'
    )
  ...
  const createWard = async (name: string) => {
    const wardService = new WardServiceClient(
      'https://staging-api.helpwave.de/tasks-svc'
    )
  ...
  const createRandomWard = async () =>
    createWard(`random_ward_${Date.now().toString()}`)
  ...
  return (
    <div className={tw('w-screen h-screen flex flex-col')}>
      ...
      <button onClick={() => getWard('9af360fc-f612-4752-9982-a596c62e85c7')}>
        Get ward (open console)
      </button>
